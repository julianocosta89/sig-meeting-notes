...
| `digest_template.html` | Jinja2 HTML email template used by `send_digest.py` (table-based layout, inline CSS, base64 PNG logo). |
| `cleanup_trivial_meetings.py` | One-time (re-runnable) script to delete hallucinated `summary.md` files for trivial meetings. Dry-run by default; pass `--execute` to apply deletions. |
